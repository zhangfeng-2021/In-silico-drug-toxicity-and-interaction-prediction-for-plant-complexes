...
def get_toxicity_information(cid):
    session = HTMLSession()
    new = 'https://pubchem.ncbi.nlm.nih.gov/rest/pug_view/data/compound/'+str(int(cid))+'/JSON/?heading=Toxicological Information'
...
if __name__ == "__main__":
    ...
    while similarity <=1 and similarity >=0.10:       
        ...
        dfinhouse = pd.read_csv(r"in-silico toxicity and drug interaction prediction\Toxicity and interaction mining\in house vector\Toxi_infor_in_house.csv") # Access the in-house vector
        ...
        try:
            ...
            #     df2 = pd.read_csv(fin, encoding = None)
            # df2 = df2.append(dfstore)
